...
(t/deftest sorted?-test
  ;; skip spec-check - uses <= which throws on non-numbers
...
(t/deftest sorted-by?-test
  ;; skip spec-check - causes instrumentation difficulties
...
(t/deftest list-sorted?-test
  ;; skip spec-check - sort can throw on mixed types
  (t/with-instrument :all
...
(t/deftest list-sorted-by?-test
  ;; skip spec-check - causes instrumentation difficulties
  ;; uses compare (returns int) not < or > (return boolean)
...
(t/deftest binary-search-test
  ;; skip spec-check - sorted? in spec causes issues with generation
