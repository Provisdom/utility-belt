...
(defn nil-nils
  "Applies function `f` but returns nil if any argument is nil.
   ...
   A simpler alternative to anomaly-nils when you don't need the detailed error reporting of
   anomalies. Can be used with threading macros to short-circuit on nil values.
...
(defn coalesce
  "Returns the first non-nil value, or nil if all values are nil.

   Unlike `or`, this only checks for nil, not falsiness.
   `false` is returned if it's the first non-nil value.
...
(defn default-nil
  "Returns `x` if non-nil, otherwise returns `default`.

   Unlike `(or x default)`, this only checks for nil, not falsiness.
   This means `false` is preserved rather than replaced.
...
(defn remove-nil-vals
  "Removes entries with nil values from a map.
   ...
   Returns an empty map if all values are nil.
...
(defn replace-nil-vals
  "Replaces nil values in a map with `default`.

   All entries are preserved, but nil values are replaced.
